...
from china_stock_data_api.cn_stock_util import TRADE_DETAIL_COLUMNS
...
class SinaStock(CnStockBase):
    ...
    def _parse(cls, body):
        ...
        for stock in stocks:
            ...
            result = [data_array[0]]
            result.extend(list(map(float, data_array[1:6])))
            result.extend(list(map(float, data_array[8:30])))
            result.extend([int_date.to_int_date(data_array[30]),data_array[31]])
