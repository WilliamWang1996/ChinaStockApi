...
from china_stock_data_api.cn_stock_util import SINA_STOCK_INFO_COLUMNS
from china_stock_data_api.cn_stock_util import multi_thread
...
class SinaStockInfo(SinaStock):
    ...
    def _parse(cls, body):
        ...
        for stock in stocks:
            ...
            data_array = data.split(',')[:19]
            result = []
            result.extend(data_array[0:2])
            result.extend(list(map(float, data_array[2:6])))
            result.extend(list(map(float, data_array[7:11])))
            result.extend([data_array[11]])
            result.extend(list(map(float, data_array[12:15])))
            result.extend(list(map(float, data_array[16:19])))
            ret.loc[index] = result
        return ret
